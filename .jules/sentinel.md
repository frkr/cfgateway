...
## 2025-02-14 - Prevent Server-Side Request Forgery (SSRF) in Background Workers
**Vulnerability:** The application was not verifying the protocol of dynamic URLs accessed by the background worker in `src/mq/MQDestiny.ts` and `src/mq/MQCallback.ts` during `fetch()` calls. If users controlled the content of these messages, they might have forced the worker to access internal services or arbitrary schemes (like `file:` or data URIs), resulting in a potential SSRF attack. Moreover, invalid URLs would fail the fetch request, leading to infinite retry loops as they would continually bounce.
**Learning:** URLs that are fetched based on parsed configuration or message contents must always be explicitly validated. Assuming that standard URLs will always have `http` or `https` is flawed and can allow internal probing or unintended protocols.
**Prevention:** Always parse dynamic URLs using `new URL()` within a `try...catch` block and explicitly verify that the protocol is `http:` or `https:` before invoking `fetch`. On validation failure, mark the request state as 'error' explicitly rather than allowing retries, mitigating SSRF and eliminating potential infinite loops.
