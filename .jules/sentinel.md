...
## 2025-02-14 - Fix Server-Side Request Forgery (SSRF) in Background Queues
**Vulnerability:** Background queue processors (`MQDestiny.ts` and `MQCallback.ts`) received dynamic URLs and passed them directly to `fetch()` without validating the protocol or whether the URL was parseable, risking Server-Side Request Forgery (SSRF).
**Learning:** Even if URLs are validated when saved into the database, background workers fetching from R2 and making outbound HTTP requests must independently validate the dynamic URLs (enforcing `http:` or `https:`) using `new URL()` to prevent data corruption or SSRF vectors at the point of execution. Additionally, when a URL is invalid, it is critical to fail securely (mark as 'error' in MQStore and stop processing) rather than throwing an exception that triggers an infinite retry loop.
**Prevention:** Always parse user-influenced URLs using `new URL()` inside a `try/catch` block and explicitly verify that the protocol is either `http:` or `https:` before passing them to `fetch()`. If validation fails, safely abort the operation without retries to prevent queue clogging and resource exhaustion.
